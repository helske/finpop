...
inits <- replicate(chains, list(
    ...
    sigma_drift_b = runif(1, 0.1, 0.3), sigma_drift_d = runif(1, 0.1, 0.3),
    drift_b = runif(n, 0.001, 0.01), drift_d = runif(n, 0, 0.05),
    sigma_rw_b = runif(1, 0.1,0.4), sigma_rw_d = runif(1, 0.1,0.4),
    rate_bd = runif(1, 0.2, 0.4),
    ...
    sigma_pop = abs(rnorm(1, 1000, 250))
  ), simplify=FALSE) 
...
fit <- sampling(model, data = data, 
                ...
                init = inits, 
                pars = c("sigma_drift_b", "sigma_drift_d", "rw_b","rw_d",
                         "sigma_rw_b", "sigma_rw_d",  "rate_bd", "mu", 
